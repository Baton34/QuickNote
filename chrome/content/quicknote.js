...
prompts: Components.classes["@mozilla.org/embedcomp/prompt-service;1"]
	...
	qnprint : function(n){
		if(n==0){n=QNqn.QN_Document.getElementById("qntabs").selectedIndex+1}
		...
		tx=tx.replace(/</g,"<<!---->")
		tx=tx.replace(/\t/g,"&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp")
		tx=tx.replace(/  /g,"&nbsp&nbsp")
		tx=tx.replace(/&nbsp  /g,"&nbsp&nbsp")
		tx=tx.replace(/\n/g,'<br>')
		var qnp = window.openDialog("", "", "")
		var html =qnp .document.getElementsByTagName("html")[0];
		html.innerHTML="<html><head><\/head><body><header><\/header><div>"+tx+"<\/div><\/body><\/html"
		var div = qnp .document.getElementsByTagName("div")[0];
		div.style.fontName=QN_globalvar.qnprefs.getCharPref("fontname"+n)
		if(QN_globalvar.qnprefs.getIntPref("printfontsizeonevalue")==0){div.style.fontSize=QN_globalvar.qnprefs.getIntPref("fontsize"+n) + "px "}
		else{div.style.fontSize=QN_globalvar.qnprefs.getIntPref("printfontsize") + "px"}
		var webBrowserPrint=window.QueryInterface(Components.interfaces.nsIInterfaceRequestor).getInterface(Components.interfaces.nsIWebBrowserPrint)
	...
	qnonfocus : function(){
		...
		var i;
		for(i=1; i<=6; i++){
			if(!QN_globalvar.qnprefs.getBoolPref("showclearbutton")) { window.document.getElementById("clear"+i).hidden = true;}
			else{window.document.getElementById("clear"+i).hidden = false;}
			if(!QN_globalvar.qnprefs.getBoolPref("showprintbutton")) { window.document.getElementById("print"+i).hidden = true;}
			else{window.document.getElementById("print"+i).hidden = false;}
			if(!QN_globalvar.qnprefs.getBoolPref("showsavebutton")) {window.document.getElementById("save"+i).hidden = true;}
			else{window.document.getElementById("save"+i).hidden = false;}
			if(!QN_globalvar.qnprefs.getBoolPref("showsaveasbutton")) {window.document.getElementById("save-as"+i).hidden = true;}
			else{window.document.getElementById("save-as"+i).hidden = false;}
			document.getElementById("Pad" + i).setAttribute("style","font: " +
			QN_globalvar.qnprefs.getIntPref("fontsize"+i) + "px " +
			QN_globalvar.qnprefs.getCharPref("fontname"+i) + "; " +
			"color:" + QN_globalvar.qnprefs.getCharPref("fontcolor"+i) + "; " +
			"-moz-appearance: none; background-color: " +
			QN_globalvar.qnprefs.getCharPref("bgcolortab"+i) + "; " +
		...
		if(!QN_globalvar.qnprefs.getBoolPref("showqnbutton")) { window.document.getElementById("qnoptions").hidden = true;}
...
////////////////////////////////////////////////////////////////////////////////
	...
	setupDocument : function(aDoc) {
		...
		QN_stringBundle = aDoc.getElementById("qnstrings");
	},
...
////////////////////////////////////////////////////////////////////////////////
	loadNote : function(Note) {
		var userTabpath
		try{userTabpath=QN_globalvar.qnprefs.getComplexValue("tab"+Note+"path",Components.interfaces.nsIPrefLocalizedString).data}
		catch(e){}
		if(!userTabpath)
			...
			if(QN_globalvar.qnprefs.getBoolPref("cursorposstart")){for(i=1; i<=6; i++)
		...
		} else if(!f.exists())
		{
		 FileIO.create(f);
			if (f.exists()) {
				...
				var textAlert =
					QN_stringBundle.getFormattedString("qnerr.cantCreate", [f.path])+'\n'+
					QN_stringBundle.getString("qnerr.cantCreate.hint");
...
////////////////////////////////////////////////////////////////////////////////
	loadAFile : function(userFileName, Note) {
		var textAlert;
		if(!userFileName) {
			textAlert = QN_stringBundle.getString("qnerr.noFile");
		...
		if (f.exists()) { 
			...
			textAlert = QN_stringBundle.getFormattedString("qnerr.invalidFile", [f.path]);
			QNqn.setTextareaText(textAlert, Note);
		}
		return true;
	},
...
////////////////////////////////////////////////////////////////////////////////
	loadFromFile : function(f,Note) {
		var charset = QN_globalvar.qnprefs.getCharPref("charset");
		QNqn.QN_charsLeft[Note] = QN_globalvar.qnprefs.getIntPref("autosave");
		try {
			...
			var textAlert =
				QN_stringBundle.getFormattedString("qnerr.failedLoadFile", [f.path])+'\n'+
				QN_stringBundle.getFormattedString("qnerr.failedLoadFile.hint", [charset]);
			QNqn.setTextareaText(textAlert,Note);
		}
	},
...
////////////////////////////////////////////////////////////////////////////////
	renameCurrentTab : function() {
		...
		catch(e){Name =QN_stringBundle.getString("default.notetitle")+" &"+Note}
		var currentTabName = Name;
		var displayName = QNqn.parseTabName(currentTabName)[0];
		var promptText = QN_stringBundle.getFormattedString("qn.tabRenamePrompt", [displayName]);
...
////////////////////////////////////////////////////////////////////////////////
	saveNote : function(i) {
		...
		if(p) {
			QNqn.QN_charsLeft[i = QN_globalvar.qnprefs.getIntPref("autosave")];
		}
	},
...
////////////////////////////////////////////////////////////////////////////////
	autoSave : function() {
		...
		if(QN_globalvar.qnprefs.getIntPref("autosave") < 0)
			return;
...
////////////////////////////////////////////////////////////////////////////////
	...
	var ct=document.getElementById("tabpanels").selectedIndex+1
		...
		var keyqnwith=false
		if((navigator.platform=="Win32" || navigator.platform=="Win64") && event.altKey==true){keyqnwith=true}
...
////////////////////////////////////////////////////////////////////////////////
	saveNoteAs : function(Note) {
		var promptText = QN_stringBundle.getFormattedString("qn.saveAsPrompt", [Note]);
...
////////////////////////////////////////////////////////////////////////////////
	saveAFile : function(aUrl, Note) {
		...
		if(!f) {
			var alertText = QN_stringBundle.getFormattedString("qnerr.cantOpenw", [aFile]);
		...
		if(data)
			p = FileIO.write(f,data,'w',QN_globalvar.qnprefs.getCharPref("charset"));
		else
...
// insert some text into current note at cursor position
	QN_insertText : function(aText) {
		...
		try {
			...
			dump(QN_stringBundle.getString("qnerr.insertText"));
			dump(e+"\n")
		}
	},
...
// here we process the TAB key (insert '\t' char ven VK_TAB is pressed)
	...
	hideSelf : function() {
		...
		if(loc == "chrome://quicknote/content/quicknote.xul") // floating window
			window.blur();
		else 
			return true;
		return false;
	...
	Print : function() {
		//floating window
		var re = [];
		window.openDialog("chrome://quicknote/content/qnprint.xul", "QN Print","chrome,modal,centerscreen",re,QNqn.getTextareaText(1), QNqn.getTextareaText(2), QNqn.getTextareaText(3), QNqn.getTextareaText(4), QNqn.getTextareaText(5), QNqn.getTextareaText(6));
	},
	////////////////////////////////////////////////////////////////////////////////
	...
	FindQNSidebar : function(aDoc)
	{
		...
		if(!this._sidebar_is_hidden(aDoc))
		{
			...
			if(currentPanel.indexOf('chrome://quicknote/content/quicknote.xul') >-1)
			{
	...
	FindQNTab : function(aDoc)
	{
		...
		var index = 0;
		while (tempTab) {
			if (browser.browsers[index].contentDocument.location=="chrome://quicknote/content/quicknote.xul") {
