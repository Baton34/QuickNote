...
////////////////////////////////////////////////////////////////////////////////
	SendSelectedTextToQNTb: function(aText) {
		...
		if(QN_globalvar.qnprefs.getIntPref("openqn")==2){
			...
			if (tab.mode.type == "contentTab"&&tab.browser.contentDocument.location=="chrome://quicknote/content/quicknote.xul") {tb=1
	...
	qnCopyToTb: function() {
		...
		var copiedToQN2 = copiedToQN + "--" + window.content.location.href;
	...
	qnCopyToTbCompose: function() {
		...
		var qnshowurl = QN_globalvar.qnprefs.getBoolPref("showUrl");
		copiedToQN= document.commandDispatcher.focusedWindow.getSelection().toString()
		if(!qnshowurl || qnshowurl == true){copiedToQN = copiedToQN + "--" + window.content.location.href;}
			//alert("Cool Sent text");
			var qndoc = QNqn.FindQNFloat();
			if(qndoc) {
				var qnWindow = qndoc.defaultView;
			...
			else{QNtb.openQNFloat(copiedToQN);}
	},
...
////////////////////////////////////////////////////////////////////////////////
	...
	openQuickNote: function( aText) {
	 ...
	 if(openMethod==1) {
		...
		try{
			...
			for (var i = 0; i < tabmail.tabInfo.length; i++) {
			var tab = tabmail.tabInfo[i];
			if (tab.mode.type == "contentTab"&&tab.browser.contentDocument.location=="chrome://quicknote/content/quicknote.xul") {
	...
	openQNFloat : function(aText) {
		if(QN_globalvar.qnprefs.getBoolPref("dependent")){window.openDialog("chrome://quicknote/content/quicknote.xul","_blank", "chrome,all,dialog=no,dependent", aText);}
		else{window.openDialog("chrome://quicknote/content/quicknote.xul","_blank", "chrome,all,dialog=no", aText);}
		window.parent.addEventListener("close", function() {if(QN_globalvar.qnprefs.getBoolPref("closechild")){if(QNtb.getQNWinByType("quicknote:mainwindow")!=null){QNtb.getQNWinByType("quicknote:mainwindow").close()}}}, false);
	},
	...
	openQuickNoteInTab: function(aText){
		...
		if(tb==0){
			if(QN_globalvar.qnprefs.getBoolPref("tabBackground")==false){var qntbtab=document.getElementById("tabmail").openTab("contentTab", {contentPage: 'chrome://quicknote/content/quicknote.xul'})}
			else{var qntbtab=document.getElementById("tabmail").openTab("contentTab", {contentPage: 'chrome://quicknote/content/quicknote.xul', background: true})}
			 if(aText){
				 setTimeout(function(){var target = QN_globalvar.qnprefs.getIntPref("totabint");
				 var Pad = qntbtab.browser.contentDocument.getElementById("Pad" + target);
				var tabmail = document.getElementById("tabmail");
				for (var i = 0; i < tabmail.tabInfo.length; i++) {
					var tab = tabmail.tabInfo[i];
					var cibn=function(){if(qntbtab.browser.contentDocument!=undefined){qntbtab.browser.contentDocument.getElementById("Pad"+target).focus()}}
					if (tab.mode.type == "contentTab"&&tab.browser.contentDocument.location=="chrome://quicknote/content/quicknote.xul") {
					...
					if(QN_globalvar.qnprefs.getBoolPref("cursorposstart")){Pad.value = aText + "\n" + "\n" + Pad.value;Pad.setSelectionRange(aText.length, aText.length)}
				else{Pad.value = Pad.value + "\n" + "\n" + aText;Pad.setSelectionRange(Pad.textLength, Pad.textLength)}},100)
				}
			}
		},

	 openQNOptions: function() {
			window.openDialog('chrome://quicknote/content/qnsettings.xul','QuickNote - Settings','chrome,toolbar,centerscreen,resizable');
	},
	...
	updateKey: function(name, select) {
		...
		if(menu) {
			if(!select)
				...
				menu.setAttribute('key', 'key_qnfocus');
		}
	},
	...
	qnaddonstatustb: function(){
		...
		if(panel!=null){
			if (QN_globalvar.qnprefs.getBoolPref("showinaddonsbar")){panel.hidden = false;}
